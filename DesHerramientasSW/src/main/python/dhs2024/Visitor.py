...
class Visitor(compiladoresVisitor):
    ...
    def visitDeclaracion(self, ctx):
        ...
        # NO GENERAMOS CÓDIGO INTERMEDIO AQUÍ (solo se genera en la asignación o DeclAsig)
        for i in range(1, ctx.getChildCount()):
            child = ctx.getChild(i)
            if hasattr(child, 'symbol') and child.symbol.type == compiladoresParser.ID:
                variable = child.getText()
                # ANTES: self.archivoCodigoIntermedio.write(f"{variable} = 0\n")
                # AHORA: Nada, si el programador no asigna, no se inicializa en 3C.
                pass
